...
-- 1-2 修改：用 Email 找到 李燕容、肌肉棒子、Q太郎，如果他的 Role 為 USER 將他的 Role 改為 
...
-- ANS 
SELECT count(*) AS 使用者數 FROM "USER";
...
-- ANS
SELECT "COURSE_BOOKING".user_id AS user_id,
	   ("CREDIT_PURCHASE".purchased_credits - COUNT("COURSE_BOOKING".status)) as remaining_credit
FROM "COURSE_BOOKING"
inner JOIN "CREDIT_PURCHASE" ON "CREDIT_PURCHASE".user_id = "COURSE_BOOKING".user_id
WHERE "COURSE_BOOKING".user_id = (SELECT id FROM "USER" WHERE name = '王小明') AND "COURSE_BOOKING".status = '上課中' 
...
-- ANS
SELECT COUNT("COACH_LINK_SKILL".skill_id) AS "coach_total",
       "SKILL".name AS "專長名稱"
FROM "COACH"
INNER JOIN "COACH_LINK_SKILL" ON "COACH".id = "COACH_LINK_SKILL".coach_id
INNER JOIN "SKILL" ON "COACH_LINK_SKILL".skill_id = "SKILL".id
...
SELECT COALESCE (SUM("CREDIT_PURCHASE".price_paid),0) AS "總營收"
...
FROM "COURSE_BOOKING"
WHERE EXTRACT(MONTH FROM "created_at") = 11 AND status in('上課中','即將授課') ;